...
ShaderProgram::init(Device device, 
	...
	if (device.m_device == nullptr) {
		...
		WARNING("ERROR: ShaderProgram::init : Error in data from params [CHECK FOR Device std::vector<D3D11_INPUT_ELEMENT_DESC> Layout.size()] \n");
		exit(1);
	}
...
ShaderProgram::CreateVertexShader(Device device) {
	...
	if (FAILED(hr)) {
		...
		WARNING("ERROR: ShaderProgram::CreateVertexShader : Error in data from method [CHECK FOR device.CreateVertexShader()] \n");
		exit(1);
	}
}
...
ShaderProgram::CreatePixelShader(Device device) {
	...
	if (FAILED(hr)) {
		MessageBox(nullptr, "The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", "Error", MB_OK);
		WARNING("ERROR: ShaderProgram::CreatePixelShader : Error in data from method [CHECK FOR CompileShaderFromFile()] \n");
		exit(1);
	}
	...
	if (FAILED(hr)) {
		...
		WARNING("ERROR: ShaderProgram::CreatePixelShader : Error in data from method [CHECK FOR device.CreatePixelShader()] \n");
		exit(1);
	}
}
